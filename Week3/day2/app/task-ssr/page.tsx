...
export default async function TaskSSRPage() {
    ...
    const tasks: Task[] = await getAllTasks(accessToken);

    return (
        <div className="max-w-3xl mx-auto py-8">
            <h1 className="text-3xl font-bold mb-6 text-center">SSR Task List</h1>
            <div className="grid gap-6">
                {tasks.map((task) => (
                    <Link href={`/task-isr/${task.id}`} key={task.id} className="no-underline">
                        <div
                            key={task.id}
